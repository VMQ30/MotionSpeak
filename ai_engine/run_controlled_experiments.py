...
def build_model_a(input_shape):
    """Model A: Current Conv1D -> Flatten -> Dense architecture"""
    return Sequential([
...
def build_model_b(input_shape):
    """Model B: Temporal Convolution + GlobalAveragePooling1D"""
    return Sequential([
        Input(shape=input_shape),
...
def augment_handedness(X_data, y_data):
    """Mirror single-handed gestures by swapping LH (99..161) and RH (162..224) slots."""
    X_aug = X_data.copy()
    y_aug = y_data.copy()
    ...
    return np.concatenate([X_aug, X_mirrored]), np.concatenate([y_aug, y_aug])

def evaluate_cross_val(X, y, groups, model_builder, use_handedness_aug=False, ablation_mode="full"):
    ...
    for fold, (train_idx, test_idx) in enumerate(gkf.split(X, y, groups)):
        X_train, y_train = X[train_idx], y[test_idx] # split
        ...
        # Apply ablation transform to test set if specified
        ...
        model.fit(X_train_f, y_train_cat, validation_data=(X_test_f, y_test_cat),
                  epochs=60, batch_size=16, verbose=0, callbacks=[early_stop])
    ...
    # Active gesture macro F1 (excluding background at index 15)
...
def main():
    print("=" * 80)
    print("RUNNING CONTROLLED MODEL IMPROVEMENT & ABLATION EXPERIMENTS")
    print("=" * 80)
    ...
    print(f"Unique signer groups: {len(np.unique(groups))}")

    # Experiment 1: Compare Model A (Current) vs Model B (Temporal Conv) vs Model C (Bi-GRU)
    print("\n--- EXPERIMENT 1: ARCHITECTURE COMPARISON (5-Fold Group Cross-Val) ---")
    res_a = evaluate_cross_val(X, y, groups, build_model_a, use_handedness_aug=False)
    print(f"Model A (Current Conv1D+Flatten): Active Macro F1={res_a['macro_active_f1']*100:.2f}% | NO F1={res_a['no_f1']*100:.1f}% (Rec={res_a['no_rec']*100:.1f}%), AFT F1={res_a['aft_f1']*100:.1f}% (Rec={res_a['aft_rec']*100:.1f}%) | AFT->NO={res_a['aft_to_no']}, GOODBYE->GOOD={res_a['goodbye_to_good']}, BG->WHAT={res_a['bg_to_what']}")

    res_b = evaluate_cross_val(X, y, groups, build_model_b, use_handedness_aug=False)
    print(f"Model B (Temporal Conv+GAP):      Active Macro F1={res_b['macro_active_f1']*100:.2f}% | NO F1={res_b['no_f1']*100:.1f}% (Rec={res_b['no_rec']*100:.1f}%), AFT F1={res_b['aft_f1']*100:.1f}% (Rec={res_b['aft_rec']*100:.1f}%) | AFT->NO={res_b['aft_to_no']}, GOODBYE->GOOD={res_b['goodbye_to_good']}, BG->WHAT={res_b['bg_to_what']}")

    res_c = evaluate_cross_val(X, y, groups, build_model_c, use_handedness_aug=False)
    print(f"Model C (Conv1D + Bi-GRU + GAP):  Active Macro F1={res_c['macro_active_f1']*100:.2f}% | NO F1={res_c['no_f1']*100:.1f}% (Rec={res_c['no_rec']*100:.1f}%), AFT F1={res_c['aft_f1']*100:.1f}% (Rec={res_c['aft_rec']*100:.1f}%) | AFT->NO={res_c['aft_to_no']}, GOODBYE->GOOD={res_c['goodbye_to_good']}, BG->WHAT={res_c['bg_to_what']}")

    # Experiment 2: Handedness Mirror Augmentation Impact
    print("\n--- EXPERIMENT 2: HANDEDNESS AUGMENTATION IMPACT ---")
    res_b_aug = evaluate_cross_val(X, y, groups, build_model_b, use_handedness_aug=True)
    print(f"Model B + Handedness Aug:         Active Macro F1={res_b_aug['macro_active_f1']*100:.2f}% | NO F1={res_b_aug['no_f1']*100:.1f}% (Rec={res_b_aug['no_rec']*100:.1f}%), AFT F1={res_b_aug['aft_f1']*100:.1f}% (Rec={res_b_aug['aft_rec']*100:.1f}%) | AFT->NO={res_b_aug['aft_to_no']}, GOODBYE->GOOD={res_b_aug['goodbye_to_good']}")

    # Experiment 3: Temporal Ablation Suite on Model B
    print("\n--- EXPERIMENT 3: TEMPORAL ABLATION SUITE ON MODEL B ---")
    for mode in ["full", "first_frame", "mid_frame", "last_frame", "shuffled"]:
        res_abl = evaluate_cross_val(X, y, groups, build_model_b, use_handedness_aug=False, ablation_mode=mode)
