...
def analyze_user_exp2():
    ...
    step_frames = max(1, int(round(fps * 0.075))) # 75ms polling
    ...
    while cap.isOpened():
        ...
        if frame_idx % step_frames == 0:
            ...
            if has_pose:
                ...
                # MotionSpeakAIModule.kt line 341 check:
            ...
            top1_str, top2_str, top3_str = "N/A", "N/A", "N/A"
            max_prob = 0.0
            ...
            if len(frame_history) >= 3:
                ...
                top3_str = f"{TARGET_GLOSSES[top[2][0]]} ({top[2][1]*100:.1f}%)"
    ...
    full_output = "\n".join(log_lines)
    with open("user_exp2_trace_log.txt", "w") as f:
        f.write(full_output)

    print(f"Trace written to user_exp2_trace_log.txt ({len(log_lines)} lines)")

    pose_valid_cnt = sum(1 for r in trace_logs if r['is_pose_valid'])
    hand_cnt = sum(1 for r in trace_logs if r['has_hand'])

    print("\n--- SUMMARY FOR user_exp2.mp4 ---")
