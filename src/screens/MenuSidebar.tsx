...
const menuItems = [
  ...
  {key:'notification',icon:require('../assets/notification.png'),text:language==='english'?'Notifications':'Mga Notification',
    onPress:()=>{closeMenu();navigation.navigate('Notifications');}},
  {
    ...
    text: language==='english' 
      ...
      : `Bluetooth: ${isConnected ? 'Konektado' : 'Hindi Konektado'}`,
    onPress:()=>{closeMenu();navigation.navigate('BluetoothStatus');}
  },
];
        ...
        {menuItems.map((item)=>(
          <TouchableOpacity key={item.key} style={[styles.menuButton,isTablet&&styles.menuButtonTablet,isTabletLandscape&&styles.menuButtonTabletLandscape]} onPress={item.onPress}>
            <Image source={item.icon} style={[
              ...
              isTabletLandscape&&styles.menuIconTabletLandscape,
              { 
                tintColor: item.key === 'bluetooth' && isConnected 
                  ? '#0086b3'
                  : (isDarkMode ? '#fff' : '#0086b3')
              }
            ]} resizeMode="contain"/>
            <Text style={[
              styles.menuText,
              ...
              getTextStyle(16),
              { 
                color: item.key === 'bluetooth' && isConnected 
                  ? '#0086b3'
                  : (item.key === 'bluetooth' && !isConnected 
                      ? '#808080'
                      : (isDarkMode ? '#fff' : '#0086b3')
                    )
              }
